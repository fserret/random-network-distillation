...
class CnnGruPolicy(StochasticPolicy):
    def __init__(self, scope, ob_space, ac_space,
        ...
        if dynamics_bonus:
            ...
            self.define_self_prediction_rew(convfeat=convfeat, rep_size=rep_size, enlargement=enlargement,scope=scope)
    ...
    def define_self_prediction_rew(self, convfeat, rep_size, enlargement,scope):
        #RND.
        ...
        #        
        yes_gpu = any(get_available_gpus())
        with tf.variable_scope(tf.get_variable_scope(), reuse=True), tf.device('/gpu:0' if yes_gpu else '/cpu:0'):
            X_im=np.load(os.getcwd()+'/policies/obs.npy')
            Xr_im = tf.cast(X_im, tf.float32) / 255.
            Xr_im = tf.reshape(Xr_im, (-1, *ph.shape.as_list()[-3:]))[:, :, :, -1:]
            Xr_im = tf.clip_by_value((Xr_im - tf.reduce_mean(Xr_im)) / (tf.math.reduce_std(Xr_im) ** 0.5), -5.0, 5.0)
        ...
        # Predictor network.
        for ph in self.ph_ob.values():
            if len(ph.shape.as_list()) == 5:  # B,T,H,W,C ###Batch time height width color?
                logger.info("CnnTarget: using '%s' shape %s as image input" % (ph.name, str(ph.shape)))
                xrp = ph[:,1:]
